  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKENDS[backend]}/employees?filterKey=${filterKey}&filterValue=${filterValue}&sortKey=${sortKey}`
      );
    ...
    <div className="min-h-screen bg-gray-950 text-white p-8">
      ...
      <div className="flex gap-2 mb-6">
        {Object.keys(BACKENDS).map(b => (
          <button
            key={b}
            onClick={() => { setBackend(b); setResults([]); }}
            ...
            <thead>
              <tr className="bg-gray-800">
                {Object.keys(results[0]).map(k => (
                  <th key={k} className="px-4 py-2 text-left text-gray-400">{k}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {results.map(emp => (
                <tr key={emp.id} className="border-t border-gray-800 hover:bg-gray-900">
